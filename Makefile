...
CFLAGS += -Wall -std=c++11 -U_FORTIFY_SOURCE $(INCLUDES) $(DEFINES)
...
all: $(BINDIR)/vm 
apps: $(BINDIR)/hello.so $(BINDIR)/sleep.so $(BINDIR)/file.so $(BINDIR)/file2.so $(BINDIR)/thread.so $(BINDIR)/mutex.so $(BINDIR)/memory.so $(BINDIR)/preempt.so $(BINDIR)/badprogram.so
